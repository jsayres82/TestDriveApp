...
namespace Data
{
    ...
    public static class Db
    {
        ...
        {
            SqlDataAdapter da = new SqlDataAdapter("SELECT * FROM [Vehicles] order by [Make]", ConfigurationManager.ConnectionStrings["daypilot"].ConnectionString);
        ...
        public static IEnumerable<SelectListItem> GetVehicleSelectList()
        {
            return
                GetVehicles().AsEnumerable().Select(u => new SelectListItem
                {
                    Value = Convert.ToString(u.Field<int>("VehicleId")),
                    Text = u.Field<string>("Make")
                });
        }
        ...
        public static void CreateReservation(DateTime start, DateTime end, int vehicle, int customer)
        {
            using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["daypilot"].ConnectionString))
            {
                con.Open();
                SqlCommand cmd = new SqlCommand("INSERT INTO [Reservation] ([ReservationStart], [ReservationEnd], [VehicleId], [CustomerId], [ReservationStatus]) VALUES (@start, @end, @resource, @name, 0)", con);
                cmd.Parameters.AddWithValue("start", start);
                cmd.Parameters.AddWithValue("end", end);
                cmd.Parameters.AddWithValue("resource", vehicle);
